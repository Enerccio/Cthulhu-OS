...
#define __SUPPORTED_LOCALE_COUNT 0

static locale_t std_locale;
...
char* setlocale(int category, const char* locale){
	locale_t* newlocale;
	if (locale == NULL || strcmp("", locale) == 0 || strcmp(__DEFAULT_LOCALE, locale) == 0 ){
		newlocale = &std_locale;
	} else {
		for (int16_t loc_id = 0; loc_id < __SUPPORTED_LOCALE_COUNT; loc_id++){
			if (strcmp(locale, localenames[loc_id])){
	...
	switch (category){
	case LC_COLLATE: return __LC_COLLATE;
	case LC_CTYPE: return __LC_CTYPE;
	case LC_MONETARY: return __LC_MONETARY;
	case LC_NUMERIC: return __LC_NUMERIC;
	case LC_TIME : return __LC_TIME;
	default:
	case LC_ALL: return __LC_ALL;
	}
